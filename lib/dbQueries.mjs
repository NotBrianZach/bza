...
export async function insertBookmark({
  ...
  function buildTimidInsertQuery(tableOperationString, argObject) {
    ...
    const builtQuery = `${tableOperationString} (
  ${filteredQueryArgs.map(val => `"${val}"`).join(", ")}
) values (${filteredQueryArgs
      .map((_, i) => "$" + (i + 1))
      .join(", ")}) on conflict do nothing`;

    return { builtQuery, filteredQueryValues };
  }

  const queryObj = buildTimidInsertQuery("insert into bookmarks", {
    ...arguments[0],
    title: undefined,
