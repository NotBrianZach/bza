...
// export function loadPDFBookmark(bTitle) {
//   const stmt = db.prepare(
//     "SELECT * FROM bookmarks join pdfs using bTitle, tStamp where bTitle=? order by tStamp limit 1"
//   );
//   const bookmark = stmt.get(bTitle);
//   console.log("bookmark", bookmark);
//   return bookmark;
...
export function loadPDFTable(bTitle) {
  const stmt = db.prepare(
    "SELECT * FROM pdfs where bTitle=? order by tStamp limit 1"
  );
